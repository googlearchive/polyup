...
import ElementMetadata from './element_metadata';
...
const domProperties = new Set([
    ...
    'parentNode', 'firstChild', 'lastChild', 'firstElementChild',
    'lastElementChild', 'previousSibling', 'nextSibling', 'textContent',
    'innerHTML', 'querySelector', 'querySelectorAll', 'getDistributedNodes',
    'getDestinationInsertionPoints', 'setAttribute', 'removeAttribute',
    'classList'
]);

class Script {
  constructor(jsSource, elements, filename, initialIndent, implicitElemName, onlyReformat) {
  ...
  upgrade() {
    ...
    try {
      ...
      throw new Error(`
Unable to parse javascript beginning with: ${this.jsSource.subString(0, 50).replace(/\n/g,'\\n')}...
...
`);
  ...
  upgradePolymerCall(polyCall) {
    ...
    const elementMetadata =
      this.elements.get(name) || new ElementMetadata(name);
    ...
    while (declaration.type === 'CallExpression') {
      ...
      if (declaration.callee.type === 'MemberExpression') {
        ...
        if (memberExpr.object.name === 'Polymer' &&
          ...
          elementMetadata.behaviors.push(mixin);
        }
      }
    }
    ...
    migrateAttributesToPropertiesBlock(
        polyCall, declaration, elementMetadata);

    // hostAttributes
    var hostAttrsProperties = [];
    for (let key in elementMetadata.hostAttrs) {
      let astVal = {type: 'Literal', value: elementMetadata.hostAttrs[key]};
    ...
    for (let key in elementMetadata.listeners) {
      listenerProperties.push({
        type: 'Property',
        key: {type: 'Identifier', name: key},
        value: {type: 'Literal', value: elementMetadata.listeners[key]}
      });
    }
    ...
    // Add behaviors
    if (elementMetadata.behaviors.length > 0) {
      declaration.properties.unshift(
        getBehaviorsDeclaration(elementMetadata.behaviors));
    }
    ...
    elementMetadata.newDeclarations.forEach(function(decl) {
    ...
    if (elementMetadata.polyfillTokenList) {
    ...
    for (const prop of declaration.properties) {
      if (prop.value.type === 'FunctionExpression') {
        this.upgradeFunctionExpression(prop.value, elementMetadata);
      }
    }
  }
  ...
  upgradeFunctionExpression(funcExpr) {
    // Upgrade assignments to properties on `this` to use `this.set`:
    const assignmentExpressions = [];
    estree_walker.walk(funcExpr.body, {
      enter: function(node) {
        // Don't upgrade inner functions.
        // TODO(rictic): Do upgrade function(){}.bind(this) expressions.
    ...
    function getPathExpr(memberExpr) {
      const pathExpr = _getPathExpr(memberExpr);
      if (!pathExpr) {
        return null;
      }
      _combineStingLiteralConcatenation(pathExpr);
      // No need to call this.set for direct properties, only nested ones.
      if (pathExpr.type === 'Literal' && !/\./.test(pathExpr.value)) {
        return null;
      }
      return pathExpr;
    }
    function _combineStingLiteralConcatenation(pathExpr) {
      if (pathExpr.type === 'BinaryExpression') {
        _combineStingLiteralConcatenation(pathExpr.left);
        _combineStingLiteralConcatenation(pathExpr.right);
      }
      // This could be smarter and combine more, but it's an edge case.
      if (pathExpr.type === 'BinaryExpression' && pathExpr.operator === '+' &&
          pathExpr.left.type === 'Literal' && pathExpr.right.type === 'Literal' &&
          (typeof pathExpr.left.value === 'string') &&
          (typeof pathExpr.right.value === 'string')) {
        pathExpr.type = 'Literal';
        pathExpr.value = pathExpr.left.value + pathExpr.right.value;
        delete pathExpr.left;
        delete pathExpr.right;
      }
    }
    function _getPathExpr(memberExpr) {
      let objExpression;
      if (memberExpr.computed) {
        objExpression = memberExpr.property;
      } else {
        objExpression = {type: 'Literal', value: memberExpr.property.name};
      }
      if (memberExpr.object.type === 'ThisExpression') {
        return objExpression;
      }
      if (memberExpr.object.type === 'MemberExpression') {
        const subExpr = _getPathExpr(memberExpr.object);
        if (!subExpr) {
          return;
        }
    ...
    for (const assignment of assignmentExpressions) {
      const pathExpr = getPathExpr(assignment.left);
      if (!pathExpr) {
        continue;
      }
      const valueExpr = assignment.right;

      // Slightly dirty, but easier than replacing the object.
      for (const prop in assignment) {
        delete assignment[prop];
      }
      ...
      assignment.arguments = [pathExpr, valueExpr];
    }
  }
    ...
    estree_walker.walk(ast, {
      enter: (node) => {
        ...
        if (node.property.type === 'Identifier' &&
            domProperties.has(node.property.name)) {
          this.modified = true;
          var object = node.object;
          node.object = {
            type: 'CallExpression',
            callee: {type: 'Identifier', name: 'Polymer.dom'},
            arguments: [object]
          };
        }
      }
...
function upgradeJs(jsSource, elements, filename, implicitElemName=null, initialIndent=0, onlyReformat=false) {
  var script = new Script(jsSource, elements, filename, initialIndent, implicitElemName, onlyReformat);
...
function migrateAttributesToPropertiesBlock(
  ...
  declaration.properties.forEach(function(property) {
    ...
    const attr = elementMetadata.getAttr(attrName);
  ...
  declaration.properties.forEach(function(property) {
    ...
    property.value.properties.forEach(function(publishedProp) {
      ...
      let attr = elementMetadata.getAttr(attrName);
  ...
  declaration.properties = declaration.properties.filter(function(prop) {
    return !(getPropertyKeyName(prop) in elementMetadata.attrs);
  });

  // Infer types from default values
  for (let attrName in elementMetadata.attrs) {
    let attr = elementMetadata.attrs[attrName];
  ...
  for (let attrName in elementMetadata.attrs) {
    const attr = elementMetadata.attrs[attrName];
    if (!attr.value) {
      continue;
    }
  ...
  declaration.properties.forEach(function(prop) {
    ...
    let attr = elementMetadata.getAttr(attrName);
    attr.observer = {type: 'Literal', value: propName};
  ...
  declaration.properties.forEach(function(prop) {
    ...
    prop.value.properties.forEach(function(computedProp) {
      ...
      const attr = elementMetadata.getAttr(computedPropName);
      const renameFn = (name) => {
        const newComputedExpression = namer(name);
        attr.computed = {type: 'Literal', value: newComputedExpression};
      };
      const initialName = '_compute' + computedPropName.charAt(0).toUpperCase() +
          computedPropName.slice(1);
      if (newDeclaration) {
        newDeclaration.onRenameHandlers.push(renameFn);
        newDeclaration.rename(initialName);
        elementMetadata.newDeclarations.add(newDeclaration);
      } else {
  ...
  if (Object.keys(elementMetadata.attrs).length === 0) {
    return;
  }
  ...
  Object.keys(elementMetadata.attrs).sort().forEach(function(attrName) {
    var attr = elementMetadata.attrs[attrName];
...
function fixupComputedExpression(expression) {
  ...
  let parsed;
  try {
    parsed = espree.parse(functionExpression);
  } catch(e) {
    throw new Error(`
