...
var needsUpgradeQuerySelectors = [];
for (var querySelector in cssMixinsUpgrades) {
  needsUpgradeQuerySelectors.push(querySelector);
}
for (querySelector in otherCssUpgrades) {
  needsUpgradeQuerySelectors.push(querySelector);
}
var needsUpgradeQuerySelector = needsUpgradeQuerySelectors.join(', ');

module.exports = {
  needsUpgradeQuerySelector: needsUpgradeQuerySelector,
  getCssRulesNeededToUpgrade: function(cheerioElement) {
    var cssRules = {};
    for (querySelector in cssMixinsUpgrades) {
