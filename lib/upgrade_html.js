...
function upgradeElementCss($, polymerElement, domModule, templateElem) {
  ...
  recursivelyMatchInsideTemplates($, $(templateElem), upgradeCss.needsUpgradeQuerySelector).forEach(function(elemNeedsUpgrade) {
    _.extend(cssRules, upgradeCss.getCssRulesNeededToUpgrade($(elemNeedsUpgrade)));
  });
  if (!_.isEmpty(cssRules)) {
    var style = $('<style>');
    style.text(getCssTextGivenRules(cssRules));
