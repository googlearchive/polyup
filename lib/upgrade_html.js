...
class Page {
  ...
  upgrade() {
    ...
    this.$('polymer-element').each((_ignored, polyElem) => {
      this.modified = true;
      const elemName = polyElem.attribs.name;
      const elementMetadata = new ElementMetadata(elemName);
    ...
    this.$('link[rel=import][href]').each((_ignored, importElem) => {
      importElem.attribs.href = this.getNewImportPath(importElem.attribs.href);
    });
  ...
  matchExpressions(str) {
    // Like espree.parse, only with a custom error message.
    const friendlyParse = (expr) => {
      try {
        return espree.parse(expr);
      } catch(e) {
        throw new Error(`
Unable to parse:   ${expr}
inside of expression:   ${str}
...
Parse error: ${e.message}
    ...
    const matchFirstExpression = (str) => {
      let match = str.match(/\[\[(.+?)\]\]/);
      ...
      for (let func of expressionPieces.slice(1)) {
        let parsed = friendlyParse(func).body[0].expression;
        if (parsed.type === 'CallExpression') {
          parsed.arguments.unshift(friendlyParse(expression).body[0].expression);
  ...
  upgradeScriptElement(scriptElem, implicitElemName) {
    if ('src' in scriptElem.attribs) {
      ...
      const upgradedJs = upgradeJs(
          scriptSource, this.elements, pathToScriptElem,
          implicitElemName, 0, this.onlyReformat);
      if (upgradedJs) {
        this.results[pathToScriptElem] = `${upgradedJs}\n`;
      }
    } else {
      const indentationDepth = this.getIndentationDepth(scriptElem);
      const upgradedJs = upgradeJs(
          this.$(scriptElem).text(), this.elements, this.filename,
          implicitElemName, indentationDepth + 1, this.onlyReformat
      );
  ...
  getIndentationDepth(element) {
    if (element.previousSibling && element.previousSibling.type == 'text') {
      var previousText = element.previousSibling.data;
  ...
  getNewImportPath(importPath) {
    let match = importPath.match(
        /core-animated-pages\/transitions\/(.*?).html$/);
    if (match) {
      const basePath = importPath.substring(0, match.index);
      const animation = elementMapping.animationMappings[match[1]];
      return `${basePath}neon-animation/animations/${animation}.html`;
    }
    match = importPath.match(/([^\/]+)\/([^\/]+)\.html$/);
    if (!match) {
      return importPath;
    }
    this.modified = true;
    ...
    const basePath = importPath.substring(0, match.index);
    return `${basePath}${newDirname}/${newFilename}.html`;
  }
}
