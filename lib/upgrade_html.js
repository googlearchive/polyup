...
class Page {
  ...
  upgradeScriptElement(scriptElem, attrs,
    ...
    if ('src' in scriptElem.attribs) {
      ...
      var pathToScriptElem = path.resolve(path.dirname(this.filename), srcPath);
      ...
      this.results[pathToScriptElem] = upgradeJs(
          scriptSource, attrs, hostAttrs, elemName, newDeclarations, 0) + '\n';
    } else {
      var upgradedJs = upgradeJs(
          this.$(scriptElem).text(), attrs, hostAttrs, elemName, newDeclarations, 1);
