...
nomnom.option('recursive', {
  flag: true,
  help: 'Upgrade all files below the given directory. Starts with any HTML ' +
      ' files found and upgrades any javascript that is referenced from them.'
});
nomnom.option('only_reformat', {
  flag: true,
  help: 'polyup necessarily makes some reformatting changes along with the ' +
      'upgrades it performs. You can get a cleaner diff by first running ' +
      'with this flag first, commit that, then run without it. When this ' +
      'flag is given polyup will only make purely formatting-related changes.'
});
...
const baseFilename = opts.path_to_html;
if (!fs.existsSync(baseFilename)) {
  console.error('No such file: ' + baseFilename);
  process.exit(66);
}
const filesToUpgrade = [];
if (!opts.recursive || !fs.statSync(baseFilename).isDirectory()) {
  filesToUpgrade.push(baseFilename);
} else {
  walk.walkSync(baseFilename, {listeners:{
    file(dirName, stat) {
      const fullPath = path.join(dirName, stat.name);
      if (fullPath.endsWith('.html')) {
        filesToUpgrade.push(fullPath);
      }
    }
  }});
}
...
opts.ignore.forEach(function(pathToIgnore) {
  pathToIgnore = path.resolve(pathToIgnore);
  if (!fs.existsSync(pathToIgnore)) {
    console.error(`Ignored file doesn't exist at \`${pathToIgnore}\``);
    process.exit(66);
  }
  toIgnore.add(pathToIgnore);
});
...
for (const filename of filesToUpgrade) {
  let outputMap;
  try {
    outputMap = upgradeHtml(filename, {
        toIgnore, webserverRoot, componentsPath,
        onlyReformat: opts.only_reformat
    });
  } catch(e) {
